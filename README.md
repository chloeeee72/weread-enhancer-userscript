...
项目使用 Vite 将模块化源码构建为**单一 `.user.js` 文件**，可直接上传 GreasyFork 分发。当前版本 `1.10.0`。**每次 `npm run build` 自动递增版本号**（如 `1.10.0 → 1.11.0`），产物文件名同步更新，保留所有历史版本于 `dist/`。
...
- **定时停止**：0–120 分钟可设，到点自动停止并提示；支持"上次定时"一键复用。
...
- **自定义阅读范围**：指定"从某文字起、到某文字止"朗读；结束文字留空则由章节续读/定时决定范围。播放前先建立预渲染 DOM 行坐标并平滑定位首字，位置稳定后才开始发声。
- 朗读滚动由连续语音字符时钟驱动：`boundary` 作为观测，自适应模型负责事件间预测，真实排版行 y 决定页面目标；`requestAnimationFrame` 闭环滚动跟随标点停顿和跨行节奏，不再按整章字符比例匀速移动。
...
- 自动阅读与语音阅读双 Tab 切换，定时、语速等状态在两个模式间联动同步。
...
│  ├─ …（1.2.0 ~ 1.9.0）
│  └─ weread.user-1.10.0.js         # 当前最新产物
├─ doc/
│  └─ TTS语音阅读合并实现方案.md     # TTS 功能合并进主脚本的设计文档
├─ src/
...
- **autoRead**：自动阅读模式保留 20ms 固定循环；语音模式委托 `scrollFollower` 使用 rAF 闭环跟随。支持定时停止、暂停/恢复和自动阅读状态恢复。
...
- **layoutMap.js / scrollFollower.js**：播放前测量真实文本行 y；播放中用 rAF 前馈 + 闭环控制滚动。重排或大误差时先暂停语音，平滑对齐后恢复。
...
### 5.1 微信读书新版阅读器是 canvas 渲染，正文没有 DOM

新版阅读器用 canvas 绘制正文，页面上的 `.readerChapterContent` 等 DOM 为空壳。要朗读正文，只能"钻"进 Vue 内部状态：遍历 Vue 实例树（`__vue__`、`__vue_app__._instance`、vnode 树）、找 Vuex store、读 `preRenderHtml` / `tempContent` 明文、调用组件内部解密方法。这是一条**依赖站点内部实现、极易随站点改版失效**的路径，因此 extractor 设计成多级降级链（预渲染 DOM → 缓存明文 → 主动解密 → 普通 DOM 兜底），并在解密前后都做"文本是否像正文"的启发式校验（长度 ≥12、含 CJK 或英文单词）。

### 5.2 翻页没有公开 API，只能模拟键盘

微信读书没有暴露翻页接口，脚本通过派发合成 `ArrowRight` 键盘事件触发翻页。难点在于：滚动到页底与翻页动作需要状态机协调（`isPageTurning` / `pageTurnCooldown` / `isWaitingForPageTurn`），合成事件不能与用户真实按键冲突，翻页后阅读器重排滚动位置，需要 1.5s 后复位滚动并重算"触底"基准，否则会重复触发或漏触发翻页。

### 5.3 SpeechSynthesis 兼容性坑多

- 语音列表**异步加载**，需要同时挂 `onvoiceschanged` 和轮询兜底；
- 部分平台的 `pause()` 不可靠（暂停后无法恢复），需降级为"取消当前 utterance 并从当前块重读"；
- 长文本单次朗读会被浏览器截断，必须按语义分块（≤220 字）；
- 浏览器可能以 `not-allowed` 拒绝无用户手势的朗读，需要提示用户先点击页面；
- 音色质量依赖系统语音包，Windows 上推荐 Edge + 微软神经网络音色。

### 5.4 全书图片扫描的性能与体验平衡

扫描整本书需要模拟滚动数千次，存在大量坑：懒加载图片未就绪、占位图混入、同一图片跨章节重复、无限滚动导致扫描停不下来、内存被图片对象撑爆。解决方案：滚动轮次上限 + 连续触底/无增长的双重终止判定；`src` 去重合并章节信息；三级 LRU 缓存（书集 → 单书图片 → 缩略图资源）并限制上限；分批渲染 + 预读（24 张/批、前探 18 张）；取消令牌（`loadToken`）保证关闭面板后旧扫描任务立即失效。

### 5.5 主题（白/深色）与护眼状态的全局联动

微信读书的深色主题是通过 `body` 加 class 切换的，脚本需要感知切换并联动：护眼模式仅在白色主题下生效、深色下自动禁用并提示；控制面板、语音快捷条的背景/文字颜色要随主题与护眼底色一起重算。使用 `MutationObserver` 监听 body class + `requestAnimationFrame` 合并抖动，避免切换瞬间出现"半套主题"。

### 5.6 GreasyFork 发布约束

- 最终产物必须是**单一 `.user.js` 文件**，所有 CSS 内联，不拆 chunk；
- 外部脚本只能引用 GreasyFork 允许的 CDN，当前仅保留 jQuery；
- 禁止远程动态注入脚本、禁止站外 `eval`（否则审核不通过）；
- 不在仓库 metadata 中固化 `@downloadURL` / `@updateURL`（由 GreasyFork 自动管理）；
- 权限声明最小化（4 个 `@grant`），存储全部走 GM API，保证在 Violentmonkey 等管理器上同样可用。

### 5.7 状态持久化与跨模块一致性

自动阅读、定时、语速、护眼、宽度、面板位置等大量状态需要跨刷新、跨模式保持一致。实现上把持久化点收敛到各模块的 save 方法（`GM_setValue`），启动时在 `appState` 读回；自动阅读与语音阅读共享语速、定时语义（通过 `pace` 换算），切换 Tab 时控件值互相同步，避免"设置了两套速度"的困惑。TTS 还做了旧 `localStorage` 数据的平滑迁移。
...
0. **`build/version-bump.js`** 自动递增 `src/metadata.txt` 中的 `@version`（递增次版本号，末位归零，如 `1.10.0 → 1.11.0`）；
...
- 大型设计先落 `doc/` 文档再实现（参考 [TTS语音阅读合并实现方案.md](doc/TTS语音阅读合并实现方案.md)）。
