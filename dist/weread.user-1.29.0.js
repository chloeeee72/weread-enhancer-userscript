...
`;
  ...
  function scoreDecoded (decoded) {
    ...
    const text = cleanDecodedText(prefixText ? prefixText + " " + htmlText : htmlText);
  ...
  function extractHtmlBody (html) {
    ...
    if (closeIndex >= 0) return html.slice(0, closeIndex);
    return html;
  }
  ...
  function rememberFirstPageEnd (chapterUid, pageCount, endIndex) {
    ...
    const prev = pageBoundaryCache.get(key);
    if (!prev || endIndex > prev.firstPageEndIndex) {
  ...
  async function extractCurrentPageContext (options = {}) {
    ...
    if (typeof document.querySelectorAll === "function") {
      ...
      if (pageTexts.length) {
        ...
        if (range) {
          const pageStartIndex = Math.max(0, Math.min(text.length, range.start));
          const pageEndIndex = Math.max(pageStartIndex + 1, Math.min(text.length, range.end));
  ...
  const voiceRead = {
    ...
    async loadAndSpeak (options = {}) {
      ...
      try {
        ...
        if (abortController.signal.aborted || sessionId !== voiceState.sessionId) return;
        const rangeResult = chunker.applyRange(chapterText, ttsSettings.rangeStart, ttsSettings.rangeEnd);
        ...
        const pageStartIndex = Math.max(0, Number(pageContext?.pageStartIndex) || 0);
    ...
    mapOffsetToScroll (offset) {
      ...
      const target = this.pageOffsetToScrollTop(safeOffset, distance);
      return Math.min(distance, Math.max(0, target + this.interpolateAnchorErr(safeOffset)));
    },
    getScrollTarget () {
    ...
    async jumpToRangeStart () {
      ...
      const pageContext = await extractor.extractCurrentPageContext({
        text: chunker.normalizeText(text),
        chapterUid: extractor.getCurrentChapterUid(),
        probe: false
      });
      if (token !== voiceState.jumpToken) return;
      const pageStartIndex = Number(pageContext?.pageStartIndex) || 0;
      const pageEndIndex = Number(pageContext?.pageEndIndex) || totalLength || 0;
      const doc = document.scrollingElement || document.documentElement;
