...
`;
  ...
  const controlPanel = {
    ...
    updateAutoReadControls() {
      ...
      $("#voiceReadTab").toggle(!isDouble);
      if (isDouble && appState.activeReadingMode === "voice") {
        appState.activeReadingMode = "auto";
        GM_setValue("weread_reading_mode", "auto");
        $("#autoReadTab").addClass("active");
        $("#voiceReadTab").removeClass("active");
        $("#autoReadPanel").show();
        $("#voiceReadPanel").hide();
      }
    },
  ...
  class SpeechClock {
    ...
    advance(now2) {
      ...
      if (this.boundarySeen && observationAge * 1e3 > SILENCE_GRACE_MS) {
        const silence = clamp((observationAge * 1e3 - SILENCE_GRACE_MS) / SILENCE_RAMP_MS, 0, 1);
  ...
  const CPS_PROBE_TIMEOUT_MS = 12e3;
  ...
  const voiceRead = {
    ...
    async loadAndSpeak(options = {}) {
      ...
      try {
        ...
        const rangeResult = usePageRangeOnly ? { text: chapterText, rangePolicy: "dynamic", startIndex: 0, endIndex: 0, totalLength: chapterText.length } : chunker.applyRange(chapterText, ttsSettings.rangeStart, ttsSettings.rangeEnd);
        ...
        if (!isDoubleColumn && !isContinuation && Number(pageContext?.pageCount) > 1) {
          const calibrated = await this.calibratePageRangeByTextLayer(chapterText, pageContext, abortController.signal);
          if (abortController.signal.aborted || sessionId !== voiceState.sessionId) return;
          const estimated = calibrated || this.estimatePageRangeByDocumentHeight(chapterText, pageContext);
          if (estimated) {
            pageStartIndex = Math.max(0, Math.min(pageStartIndex, estimated.pageStartIndex));
            pageEndIndex = Math.min(chapterText.length, Math.max(pageStartIndex + 1, estimated.pageEndIndex));
          }
        }
        ...
        if (abortController.signal.aborted || sessionId !== voiceState.sessionId) return;
        let initialLoc = null;
        if (!usePageRangeOnly && ttsSettings.rangeStart) {
          ...
          if (layoutMeasured?.layout) {
            ...
            if (Number.isFinite(yStart) && Number.isFinite(yEnd) && Number.isFinite(yTarget) && yEnd > yStart) {
              ...
              if (Number.isFinite(pageYStart) && Number.isFinite(pageYEnd) && pageYEnd > pageYStart + 1) {
                ...
                raw = this.getScrollDistance() * fraction;
              }
              hiddenTarget = Math.max(0, Math.min(this.getScrollDistance(), raw - this.getViewportFocusY()));
            }
          }
        ...
        if (!this.hasCpsProfile()) {
          const calibration = await this.calibrateSpeechRate(abortController.signal);
          if (abortController.signal.aborted || sessionId !== voiceState.sessionId) return;
          if (calibration && Number.isFinite(calibration.cpsAt1x)) {
            voiceState.calibratedCps = calibration.cpsAt1x;
            voiceState.speechClock.setFallbackCps(calibration.cpsAt1x);
            this.saveCpsProfile(calibration.cpsAt1x);
            if (calibration.sawBoundary) {
              voiceState.boundarySupported = true;
            } else {
              voiceState.boundarySupported = false;
            }
            this.debugLog("首块静音校准 cps@1x=" + calibration.cpsAt1x.toFixed(2) + " boundary=" + calibration.sawBoundary + " ms=" + Math.round(calibration.ms));
          }
        }
    ...
    calibrateSpeechRate(signal) {
      ...
      return new Promise((resolve) => {
        ...
        utterance.onend = () => {
          ...
          const cpsAt1x = CPS_PROBE_TEXT.length / (ms / 1e3) / Math.max(0.1, ttsSettings.rate);
          finish({
            ms,
    ...
    pageOffsetToScrollTop(offset, distance, pageStart = voiceState.pageStartIndex, pageEnd = voiceState.pageEndIndex) {
      ...
      if (Number.isFinite(pageYStart) && Number.isFinite(pageYEnd) && pageYEnd > pageYStart + 1) {
        const raw2 = pageYStart + fraction * (pageYEnd - pageYStart);
        return Math.max(0, Math.min(maxScroll, raw2 - focusY));
      }
    ...
    async calibratePageRangeByTextLayer(chapterText, pageContext, signal) {
      ...
      try {
        ...
        this.debugLog("页边界文本层校准 start=" + pageStartIndex + " end=" + pageEndIndex + " topY=" + (topRange?.y ?? null) + " bottomEnd=" + (bottomRange?.end ?? null));
      ...
      if (startIndex >= pageStartIndex && startIndex < pageEndIndex) {
        ...
        if (layoutMeasured?.layout) {
          ...
          if (Number.isFinite(yStart) && Number.isFinite(yEnd) && Number.isFinite(yTarget) && yEnd > yStart) {
            ...
            const target2 = Math.max(0, Math.min(distance, raw - this.getViewportFocusY()));
            window.scrollTo(0, target2);
            utils.notificationManager.show("已定位到开始文字附近");
            return;
          }
