...
`;
    ...
    try {
      if (typeof DOMParser !== "undefined") {
        ...
        if (doc && typeof doc.querySelectorAll === "function") {
          ...
          if (bodyText) return chunker.normalizeText(bodyText);
        }
      }
    } catch (error) {
    }
  ...
  function trimToHtmlStart(decoded) {
    const match = HTML_START_RE.exec(decoded || "");
    return match ? decoded.slice(match.index) : null;
  }
  function isCoverLike(decoded) {
    return /<\?xml/i.test(decoded) || /<head[\s>]/i.test(decoded);
  }
  ...
  function scoreDecoded(decoded) {
    ...
    const html = trimToHtmlStart(decoded);
    if (!html) return null;
    const noiseLength = decoded.length - html.length;
    if (isCoverLike(html) || isCssLike(html)) return null;
    const text = htmlToText$1(html);
    if (HTML_FRAGMENT_START_RE.test(text)) return null;
    if (!chunker.isLikelyChapterText(text)) return null;
    return {
      html,
      text,
      structure: htmlStructureRank(html),
      score: chunker.scoreChapterText(text) - Math.min(20, noiseLength)
    };
  }
  ...
  function clearStaleEntries(chapterUid, bookId) {
    const list = cache.get(chapterUid) || [];
    const kept = list.filter((item) => !bookId || item.bookId !== bookId);
    if (kept.length === list.length) return list;
    cache.set(chapterUid, kept);
    return kept;
  }
  function storeResponse(url, requestBody, responseBody) {
    ...
    sequence += 1;
    const list = clearStaleEntries(chapterUid, bookId);
  ...
  function getChapterText(chapterUid, options = {}) {
    ...
    const accepted = list.filter((item) => item.html && item.text && chunker.isLikelyChapterText(item.text)).sort(
      (a, b) => b.score - a.score || b.structure - a.structure || a.fetchedAt - b.fetchedAt || a.sequence - b.sequence
    );
    if (!accepted.length) return null;
    const maxScore = accepted[0].score;
    const tolerance = Number(options.tolerance ?? 12);
    const selected = accepted.filter((item) => maxScore - item.score <= tolerance).sort((a, b) => a.fetchedAt - b.fetchedAt || a.sequence - b.sequence);
    const seen = /* @__PURE__ */ new Set();
    const texts = [];
  ...
  async function waitForChapter(chapterUid, options = {}) {
    const uid = String(chapterUid || "");
    if (!uid) return null;
    const immediate = getChapterText(uid, options);
    ...
    while (Date.now() - startedAt < timeout) {
      ...
      const result = getChapterText(uid, options);
  ...
  async function extractCurrentChapterText(options = {}) {
    ...
    const apiResult = uid ? await apiChapter.waitForChapter(uid, { bookId, timeout: 3e3, interval: 80 }) : apiChapter.getChapterText("", { bookId });
