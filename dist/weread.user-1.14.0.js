...
`;
  ...
  const extractor = {
     ...
     */
    async locateTextOffset(normOffset, phrase, fromNormOffset) {
      const root = await ensurePreRenderDomNode();
      if (!root) return null;
  ...
  const speechEngine = {
    ...
    adjustChunkStartTimeAfterPause() {
      if (this.pausedAt > 0) {
        this.chunkStartTime += Date.now() - this.pausedAt;
        this.pausedAt = 0;
      }
    },
    stop() {
  ...
  const voiceRead = {
    ...
    getReadingProgress() {
      ...
      if (speechEngine.playing && !speechEngine.paused && !speechEngine.stopped && currentIndex < chunks.length) {
        ...
        const estimated = Math.max(0, elapsedSec * pace.CHARS_PER_SECOND_AT_1X * ttsSettings.rate);
    ...
    getRemainingSeconds() {
      ...
      return remainingChars / (pace.CHARS_PER_SECOND_AT_1X * ttsSettings.rate);
    },
    ...
    async locateRangeBounds(rangeResult, sessionId) {
      ...
      if (startLoc && Number.isFinite(startLoc.y)) voiceState.rangeStartY = startLoc.y;
      if (endLoc && Number.isFinite(endLoc.y)) voiceState.rangeEndY = endLoc.y;
      voiceState.locatePending = false;
    ...
    mapProgressToScroll(fraction, distance, currentTop) {
      ...
      if (Number.isFinite(voiceState.rangeStartY)) {
        const endY = Number.isFinite(voiceState.rangeEndY) ? voiceState.rangeEndY : safeDistance;
        const range = Math.max(0, endY - voiceState.rangeStartY);
        const target = voiceState.rangeStartY + range * clamped;
        return Math.min(safeDistance, Math.max(0, target));
      }
      const startF = this.getRangeStartFraction();
      const endF = this.getRangeEndFraction();
      return safeDistance * (startF + clamped * Math.max(0, endF - startF));
    },
    /** “从文字”在整章中的占比（比例估算用） */
