...
// @description  增加多功能按钮，内含多种颜色护眼模式、调整页面宽度、自动/挂机阅读、图片复制与下载
// @author       Chloe
...
`;
  ...
  const autoRead = {
    calculateWaitTime () {
      if (appState.currentScrollSpeed <= 0.5) return 10;
      if (appState.currentScrollSpeed <= 1) return 8;
      if (appState.currentScrollSpeed <= 2) return 6;
      if (appState.currentScrollSpeed <= 3) return 4;
      return 2;
    },
    start () {
      ...
      const timerMinutes = parseInt($("#timerSlider").val(), 10);
      if (timerMinutes > 0) {
      ...
      const baseSpeed = 1;
      const speedMultiplier = appState.currentScrollSpeed;
      ...
      appState.scrollInterval = setInterval(() => {
        ...
        if (currentScrollTop === lastScrollTop) {
          stuckCount += 1;
          window.scrollBy(0, baseSpeed * speedMultiplier * (stuckCount > 5 ? 3 : 1));
        } else {
          stuckCount = 0;
    ...
    updateTimerDisplay () {
      $("#timerDisplay").text(
        appState.remainingTime > 0 ? `剩余: ${Math.floor(appState.remainingTime / 60)}:${String(appState.remainingTime % 60).padStart(2, "0")}` : ""
      );
    },
    updateButton () {
    ...
    applyLastTimer () {
      ...
      $("#timerSlider").val(appState.lastTimerValue);
      $("#timerValue").text(`${appState.lastTimerValue}分钟`);
    ...
    restoreState () {
      ...
      $("#speedSlider").val(appState.currentScrollSpeed);
      $("#speedValue").text(`${appState.currentScrollSpeed.toFixed(1)}x`);
  ...
  const controlPanel = {
      ...
      <div class="control-panel" style="display: none;" id="mainControlPanel">
        ...
        <div class="control-section">
          ...
          <div class="control-item">
            ...
            <input type="range" class="control-slider speed-slider" id="speedSlider" min="0.5" max="4" step="0.1" value="${appState.currentScrollSpeed}">
            <span class="control-value" id="speedValue">${appState.currentScrollSpeed.toFixed(1)}x</span>
          </div>
          <div class="control-item">
          ...
          <div class="control-buttons">
            <button class="control-btn" id="toggleAutoRead">${appState.isAutoReading ? "停止阅读" : "开始阅读"}</button>
    ...
    bindEvents () {
      ...
      $("#speedSlider").on("input", function handleSpeedInput () {
        appState.currentScrollSpeed = parseFloat($(this).val());
        $("#speedValue").text(`${appState.currentScrollSpeed.toFixed(1)}x`);
        GM_setValue("weread_scroll_speed", appState.currentScrollSpeed);
        if (appState.isAutoReading) {
      ...
      $("#lastTimerBtn").on("click", () => moduleRegistry.autoRead?.applyLastTimer());
      $("#toggleAutoRead").on("click", () => moduleRegistry.autoRead?.toggle());
