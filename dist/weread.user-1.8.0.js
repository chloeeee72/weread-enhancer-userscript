...
`;
  ...
  const autoRead = {
    ...
    beginScroll() {
      ...
      appState.scrollInterval = setInterval(() => {
        ...
        const scrollStep = appState.activeReadingMode === "auto" ? pace.getScrollStepFromDuration(appState.readingDuration, scrollHeight, clientHeight) : pace.getScrollStepFromPage(appState.currentScrollSpeed, scrollHeight, clientHeight);
  ...
  function getSnapPosition(bar, left, top) {
    ...
    const nextLeft = edge === "left" ? 0 : edge === "right" ? viewportWidth - width : clamp(left, 0, viewportWidth - width);
