...
body:not(.wr_whiteTheme) .slider-ticks i {
  background: #666;
}

body:not(.wr_whiteTheme) .slider-ticks i.major {
  background: #4CAF50;
}
...
`;
  ...
  const appState = {
    ...
    readingDuration: GM_getValue("weread_reading_duration", 8),
    isPageTurning: false,
  ...
  const pace = {
    clampRate(value) {
      ...
      return Math.min(RATE_MAX, Math.max(RATE_MIN, number));
    },
    clampDuration(value) {
      const number = Number(value);
      if (!Number.isFinite(number)) return 8;
      return Math.min(DURATION_MAX, Math.max(DURATION_MIN, number));
    },
    getRateFromDuration(duration) {
      return Math.min(RATE_MAX, Math.max(RATE_MIN, 8 / this.clampDuration(duration)));
    },
    getDurationFromRate(rate) {
      return Math.min(DURATION_MAX, Math.max(DURATION_MIN, 8 / this.clampRate(rate)));
    },
    getScrollStep(rate) {
      return Math.max(1, this.clampRate(rate));
    },
    getScrollStepFromDuration(duration) {
      return Math.max(1, this.getRateFromDuration(duration));
    },
    getPageTurnWaitSeconds(rate) {
      return Math.min(16, Math.max(1.5, 8 / this.clampRate(rate)));
    },
    getPageTurnWaitFromDuration(duration) {
      return Math.min(16, Math.max(1.5, this.clampDuration(duration)));
    },
    applyRate(rate) {
  ...
  const autoRead = {
    ...
    beginScroll() {
      ...
      const scrollStep = appState.activeReadingMode === "auto" ? pace.getScrollStepFromDuration(appState.readingDuration) : pace.getScrollStep(appState.currentScrollSpeed);
    ...
    updateLastTimerButton() {
      $("#lastTimerBtn, #autoLastTimerBtn").each(function updateLastTimer() {
        const disabled = appState.lastTimerValue <= 0;
        $(this).toggleClass("disabled", disabled).css("background", disabled ? "#cccccc" : "#e0e0e0");
      });
    },
    applyLastTimer() {
      ...
      this.setTimerValue(appState.lastTimerValue);
      utils.notificationManager.show(`已设置为上次定时时间: ${appState.lastTimerValue}分钟`);
    },
    setTimerValue(minutes) {
      $("#timerSlider").val(minutes);
      $("#autoTimerSlider").val(minutes);
    ...
    syncTimerSliders() {
      ...
      $("#timerSlider").val(nextValue);
      $("#autoTimerSlider").val(nextValue);
      $("#timerValue").text(`${nextValue}分钟`);
      $("#autoTimerValue").text(`${nextValue}分钟`);
    },
    getReadingDuration() {
    ...
    init(panel) {
      ...
      panel.on("mousedown", function handleMouseDown(event) {
        const interactiveControl = $(event.target).closest(
          "button, input, select, textarea, .color-option, .control-btn, .control-select, .range-input"
        );
  ...
  const controlPanel = {
        ...
        <div class="control-section">
          ...
          <div class="reading-panel" id="autoReadPanel">
            ...
            <div class="control-item">
              ...
              <div class="slider-box">
                <input type="range" class="control-slider duration-slider" id="readingDurationSlider" min="1" max="60" step="1" value="${appState.readingDuration}">
                <div class="slider-ticks" aria-hidden="true"></div>
              </div>
            ...
            <div class="control-buttons">
              <button class="control-btn secondary" id="autoLastTimerBtn">上次定时</button>
          ...
          <div class="reading-panel" id="voiceReadPanel" style="display:none;">
            <div class="control-item">
              ...
              <div class="slider-box">
                <input type="range" class="control-slider speed-slider" id="ttsRateSlider" min="0.5" max="4" step="0.1" value="${appState.currentScrollSpeed}">
                <div class="slider-ticks" aria-hidden="true"></div>
              </div>
    ...
    syncModeControls() {
      if (appState.activeReadingMode === "auto") {
        const duration = pace.getDurationFromRate(appState.currentScrollSpeed);
    ...
    addSliderTicks() {
      $(".slider-box").each((_, box) => {
        const slider = $(box).find('input[type="range"]')[0];
        const min = parseFloat(slider.min);
        const max = parseFloat(slider.max);
        const step = parseFloat(slider.step || "1");
        const tickStep = this.getSliderTickStep(min, max, step);
        const start = Math.ceil(min / tickStep) * tickStep;
        const ticks = [];
        for (let value = start; value <= max + 1e-9; value += tickStep) {
          const percent = (value - min) / (max - min) * 100;
          ticks.push(`<i class="${Number.isInteger(value) ? "major" : ""}" style="left:${percent}%"></i>`);
        }
        $(box).find(".slider-ticks").html(ticks.join(""));
      });
    },
    getSliderTickStep(min, max, step) {
      const range = max - min;
      if (range <= 6) return step < 1 ? 1 : step;
      if (range <= 60) return 5;
      if (range <= 120) return 10;
      return Math.max(step, Math.round(range / 8 / step) * step);
    },
    addControlButton() {
    ...
    bindEvents() {
      ...
      $("#widthSlider").on("input", function handleWidthInput() {
        const newWidth = parseInt($(this).val(), 10);
      ...
      $("#readingDurationSlider").on("input", function handleDurationInput() {
        const duration = pace.clampDuration(parseInt($(this).val(), 10));
        appState.readingDuration = duration;
        GM_setValue("weread_reading_duration", duration);
        $("#readingDurationValue").text(`${duration}秒/页`);
        moduleRegistry.autoRead?.syncPace();
      ...
      $("#timerSlider").on("input", function handleTimerInput() {
        const minutes = parseInt($(this).val(), 10) || 0;
        $("#timerValue").text(`${minutes}分钟`);
        $("#autoTimerSlider").val(minutes);
        $("#autoTimerValue").text(`${minutes}分钟`);
        moduleRegistry.autoRead?.updateTimerDisplay?.();
  ...
  const voiceRead = {
    ...
    bindControlEvents() {
      ...
      $(document).on("input", "#ttsRateSlider", function handleRateInput() {
        ...
        if (Math.abs(rate - nearestInteger) <= 0.12) {
          rate = nearestInteger;
          $(this).val(rate);
        }
