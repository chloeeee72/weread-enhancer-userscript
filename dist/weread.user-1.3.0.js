  ...
  const panelDrag = {
    init(panel) {
      ...
      panel.on("mousedown", function handleMouseDown(event) {
        if ($(event.target).is("button, input, .color-option, .control-btn") || $(event.target).closest("button, input, .color-option, .control-btn").length) {
          return;
        }
        isDragging = true;
  ...
  const voiceRead = {
    ...
    bindControlEvents() {
      ...
      $(document).on("input", "#ttsRateSlider", function handleRateInput() {
        voiceRead.setRate(parseFloat($(this).val()));
      });
