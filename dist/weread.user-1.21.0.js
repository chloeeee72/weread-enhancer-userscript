      ...
      if (typeof el.cloneNode === "function") {
        ...
        if (typeof clone.querySelectorAll === "function") {
          clone.querySelectorAll(
            "script, style, noscript, svg, canvas, audio, video, iframe, button, input, textarea, select, a"
  ...
  function getLegacyDomText() {
    ...
    if (hasCanvas) {
      for (const selector of TEXT_LAYER_SELECTORS) {
        ...
        const text = chunker.normalizeText(nodes.map((node) => node.innerText || node.textContent || "").join(" "));
        if (chunker.isPlausibleText(text)) return { text, source: "DOM:textLayer" };
      }
      const preRender = capturePreRenderDom(findAppElement());
      if (preRender && chunker.isPlausibleText(preRender.text)) {
