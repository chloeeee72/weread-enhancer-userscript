...
`;
      ...
      if (typeof pageWindow.Function === "function") {
        const bootstrap = pageWindow.Function(
          ...
          [
            ...
            "  inspect(cachedModule && cachedModule.exports, 2);",
  ...
  function refreshReaderContext() {
    ...
    let source = "Vue";
    if (!store || !readerVms.some((candidate) => typeof candidate.decryptRenderHtml === "function")) {
  ...
  function rememberResult(result) {
    ...
    if (!chunker.isPlausibleText(text)) return result;
  ...
  function findCachedPlaintext(instances, uid) {
    ...
    for (const vm of instances) {
      ...
      for (const candidate of candidates) {
        ...
        const score = candidate.key === "preRenderHtml" ? 0 : 1;
  ...
  function collectEntries(readerState, vm, currentUid) {
    const result = [];
    const seen = /* @__PURE__ */ new Set();
    ...
    if (readerState) {
      add(readerState.horizontalReaderChapterContentHtml, currentUid);
      add(readerState.chapterContentHtml, currentUid);
      Object.keys(readerState).forEach((key) => {
        ...
        add(readerState[key], currentUid);
      });
    }
    if (vm) {
      add(vm.horizontalReaderChapterContentHtml, currentUid);
      add(vm.chapterContentHtml, currentUid);
    }
    return result;
  }
  function getRenderedHtml(returned, vm, before, rejectedValues = []) {
    ...
    if (typeof returned === "string") candidates.push(returned);
    if (typeof vm.tempContent === "string" && vm.tempContent !== before.tempContent) {
      candidates.push(vm.tempContent);
    }
    if (typeof vm.preRenderHtml === "string" && vm.preRenderHtml !== before.preRenderHtml) {
      candidates.push(vm.preRenderHtml);
    }
    return candidates.find((html) => {
      const text = htmlToText(html);
      return chunker.isPlausibleText(text) && !rejectedTexts.has(text);
    }) || "";
  }
  async function runRenderer(vm, action, rejectedValues) {
    const before = {
      tempContent: typeof vm.tempContent === "string" ? vm.tempContent : "",
      preRenderHtml: typeof vm.preRenderHtml === "string" ? vm.preRenderHtml : ""
    };
  ...
  async function decryptEntry(vms, entry, uid, index, probe) {
    ...
    for (const vm of vmList) {
      if (!vm || !("isShowPreRender" in vm) || vm.isShowPreRender) continue;
  ...
  function readPreRenderDom(chapterUid) {
    ...
    if (!uid && !captured && !extractorState.cachedPreRenderNode?.isConnected) return null;
    if (isStaleTextForChapter(uid, cached.text)) return null;
    return { text: cached.text, source: cached.source, chapterUid: uid };
  }
  function getLegacyDomText() {
    ...
    for (const el of candidates) {
      ...
      if (el.querySelector("canvas, iframe, button, input, textarea, select, a")) continue;
      const text = chunker.normalizeText(el.innerText);
      if (!chunker.isPlausibleText(text)) continue;
  ...
  async function extractCurrentChapterText(options = {}) {
    ...
    for (const item of pool.slice(0, 20)) {
      ...
      if (!chunker.isPlausibleText(text2) || isStaleTextForChapter(uid, text2) || seenTexts.has(text2)) {
        continue;
      }
      seenTexts.add(text2);
      texts.push(text2);
    ...
    const cached = pool.length ? null : findCachedPlaintext(instances, uid);
    if (cached) {
      return rememberResult({
        text: cached.text,
  ...
  function findPhraseInDom(root, normPhrase, fromNormOffset) {
    ...
    return {
      node: segment.node,
      rawOffset: mapNormToRaw$1(segment.raw, localNormStart),
  ...
  function findNormOffsetInDom(root, normOffset) {
    ...
    for (const segment of segments) {
      ...
      if (segmentEnd > normOffset) {
        ...
        return { node: segment.node, rawOffset: mapNormToRaw$1(segment.raw, localOffset), normOffset };
      }
    }
    return null;
  }
  ...
  const extractor = {
    ...
    async withPreRenderDom(callback, options = {}) {
      ...
      return withPreRenderDomNode(callback, options);
    },
  ...
  function getSnapPosition(bar, left, top) {
    ...
    const nextLeft = edge === "left" ? 0 : edge === "right" ? viewportWidth - width - rightGap : clamp$2(left, 0, viewportWidth - width);
    const nextTop = edge === "top" ? 0 : edge === "bottom" ? viewportHeight - height : clamp$2(top, 0, viewportHeight - height);
  ...
  function initQuickBarDrag(bar) {
    ...
    document.addEventListener("mouseup", () => {
      ...
      if (minEdgeDistance <= EDGE_SNAP_THRESHOLD) {
        ...
        const clampedLeft = clamp$2(left, 0, Math.max(0, viewportWidth - width));
        const clampedTop = clamp$2(top, 0, Math.max(0, viewportHeight - height));
  ...
  class SpeechClock {
    ...
    advance(now2) {
      ...
      const dt = clamp$1((time - this.lastTickAt) / 1e3, 0, MAX_FRAME_SECONDS);
      this.lastTickAt = time;
      ...
      const desiredVelocity = clamp$1(
        baseVelocity + PHASE_GAIN * phaseError,
        0,
        Math.max(4, baseVelocity * 3)
      );
      const velocityDelta = clamp$1(
        desiredVelocity - this.velocity,
        -MAX_ACCELERATION * dt,
        MAX_ACCELERATION * dt
      );
      this.velocity = Math.max(0, this.velocity + velocityDelta);
      this.position = clamp$1(this.position + this.velocity * dt, this.rangeStart, chunk.endOffset);
      return this.position;
    }
    ...
    observeBoundary(index, charIndex, now2) {
      ...
      const time = Number(now2) || 0;
      this.advance(time);
      const local = clamp$1(Number(charIndex) || 0, 0, chunk.text.length);
      const offset = clamp$1(chunk.startOffset + local, chunk.startOffset, chunk.endOffset);
      ...
      if (this.lastObservation) {
        ...
        if (seconds > 0.04 && chars > 0) {
          const sample = clamp$1(chars / seconds, 0.1, 30);
    ...
    resetObservations(offset, now2) {
      const time = Number(now2) || 0;
      const safeOffset = clamp$1(Number(offset) || this.position, this.rangeStart, this.rangeEnd);
      this.previousObservation = null;
      this.observedVelocity = null;
    ...
    getProgress(now2) {
      ...
      return {
        offset,
        charsRead: clamp$1(offset - this.rangeStart, 0, span),
        totalChars: span,
        fraction: clamp$1((offset - this.rangeStart) / span, 0, 1)
      };
    }
  }
  ...
  function getCharacterY(node, rawOffset) {
    ...
    const y = rect.top + scrollTop;
    const maxY = Math.max(root.scrollHeight || 0, document.body?.scrollHeight || 0);
  ...
  const voiceRead = {
    ...
    waitForPageTurn(signal) {
      ...
      return new Promise((resolve) => {
        const check = () => {
          ...
          if (!appState.isPageTurning || performance.now() - startedAt > 2200) {
            resolve(true);
            return;
          }
    ...
    async extractReadableChapter(options, signal) {
      ...
      const attempts = options.continuation ? CHAPTER_LOAD_RETRY_COUNT : 3;
      for (let attempt = 0; attempt < attempts; attempt += 1) {
        ...
        try {
          ...
          if (uidMatches && !isRejectedText && chunker.isPlausibleText(normalizedText)) {
          ...
          if (uidMatches && !resultUid) {
            ...
            if (legacy && legacyText !== rejectedText && chunker.isPlausibleText(legacyText)) {
        ...
        if (attempt < attempts - 1) {
          await new Promise((resolve) => window.setTimeout(resolve, 400 + attempt * 200));
        }
      }
      return null;
    },
    async loadAndSpeak(options = {}) {
      ...
      try {
        ...
        const rangeStartOffset = rangeResult.startIndex || 0;
        const rangeEndOffset = rangeResult.endIndex > 0 ? rangeResult.endIndex : rangeStartOffset + rangeResult.text.length;
        const chunks = chunker.chunkTextWithOffsets(rangeResult.text, rangeStartOffset);
        if (!chunks.length) {
        ...
        voiceState.rangeStartIndex = rangeStartOffset;
        voiceState.rangeEndIndex = rangeEndOffset;
        ...
        voiceState.speechClock.configure({
          ...
          rangeStart: rangeStartOffset,
          rangeEnd: rangeEndOffset
        });
        await this.prepareLayoutMap(sessionId, abortController.signal);
        if (abortController.signal.aborted || sessionId !== voiceState.sessionId) return;
        await this.alignToOffset(rangeStartOffset, abortController.signal);
        if (abortController.signal.aborted || sessionId !== voiceState.sessionId) return;
    ...
    mapOffsetToScroll(offset) {
      ...
      const measuredY = getLayoutY(voiceState.layoutMap, safeOffset);
      if (Number.isFinite(measuredY)) {
        return Math.min(distance, Math.max(0, measuredY - this.getViewportFocusY()));
      }
      const linearY = this.linearMapY(safeOffset, distance);
      return Math.min(distance, Math.max(0, linearY + this.interpolateAnchorErr(safeOffset)));
    },
    getScrollTarget() {
        ...
        measured = await Promise.race([
          extractor.withPreRenderDom((root2) => {
            const layout2 = buildLayoutMap(
              root2,
              voiceState.chapterText,
              voiceState.rangeStartIndex,
              voiceState.rangeEndIndex
            );
    ...
    async jumpToRangeStart() {
      ...
      if (located && Number.isFinite(located.y)) {
        await scrollFollower.alignTo(Math.max(0, located.y - this.getViewportFocusY()));
        utils.notificationManager.show("已定位到开始文字");
        return;
      }
      if (totalLength > 0) {
        ...
        await scrollFollower.alignTo(Math.max(0, distance * fraction));
