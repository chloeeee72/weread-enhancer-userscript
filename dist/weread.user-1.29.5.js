...
`;
  ...
  const autoRead = {
    calculateWaitTime() {
      if (appState.activeReadingMode === "auto") {
        const speed = Math.min(2, Math.max(0.3, appState.autoScrollSpeed || 1));
        return pace.getPageTurnWaitFromDuration(appState.readingDuration * speed);
      }
    ...
    beginScroll() {
      ...
      appState.scrollInterval = setInterval(() => {
        ...
        const speed = Math.min(2, Math.max(0.3, appState.autoScrollSpeed || 1));
        const scrollStep = pace.getScrollStepFromSeconds(appState.readingDuration * speed, scrollHeight, clientHeight);
  ...
  const controlPanel = {
        ...
        <div class="control-section">
          ...
          <div class="reading-panel" id="autoReadPanel">
            ...
            <div class="control-item">
              ...
              <div class="slider-box">
                <input type="range" class="control-slider auto-speed-slider" id="autoScrollSpeedSlider" min="0.3" max="2" step="0.1" value="${appState.autoScrollSpeed}">
              </div>
    ...
    snapSliderValues() {
      ...
      const autoSpeedValue = Math.min(2, Math.max(0.3, Math.round((parseFloat($("#autoScrollSpeedSlider").val()) || 1) * 10) / 10));
    ...
    bindEvents() {
      ...
      $("#autoScrollSpeedSlider").on("input", function handleAutoSpeedInput() {
        const speed = Math.min(2, Math.max(0.3, Math.round((parseFloat($(this).val()) || 1) * 10) / 10));
        $(this).val(speed);
  ...
  const voiceRead = {
    ...
    async loadAndSpeak(options = {}) {
      ...
      try {
        ...
        if (abortController.signal.aborted || sessionId !== voiceState.sessionId) return;
        if (isContinuation && continuationBoundary) {
      ...
      if (startIndex >= pageStartIndex && startIndex < pageEndIndex) {
        ...
        if (layoutMeasured?.layout) {
          ...
          if (Number.isFinite(yStart) && Number.isFinite(yEnd) && Number.isFinite(yTarget) && yEnd > yStart) {
            ...
            const viewport = window.innerHeight || document.documentElement?.clientHeight || 800;
            const lastScreenThreshold = Math.max(viewport * 0.5, 120);
            const target2 = raw >= distance - lastScreenThreshold ? distance : Math.max(0, Math.min(distance, raw - this.getViewportFocusY()));
            window.scrollTo(0, target2);
            utils.notificationManager.show("已定位到开始文字附近");
            return;
          }
