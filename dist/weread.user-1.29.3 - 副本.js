...
`;
  ...
  const voiceRead = {
     ...
     */
    async searchTextLayerForPhrase(phrase) {
    ...
    async jumpToRangeStart() {
      ...
      const searched = await this.searchTextLayerForPhrase(startText);
      if (token !== voiceState.jumpToken) return;
      if (searched && Number.isFinite(searched.y)) {
        await scrollFollower.alignTo(Math.max(0, searched.y - this.getViewportFocusY()));
        utils.notificationManager.show("已定位到开始文字");
        return;
      }
      ...
      if (startIndex >= pageStartIndex && startIndex < pageEndIndex) {
        const boundary = await extractor.findTextLayerBoundary(text);
