...
`;
  ...
  const VOICE_SCROLL_STEP_CAP = 30;
  const autoRead = {
    calculateWaitTime() {
    ...
    beginScroll() {
      ...
      appState.scrollInterval = setInterval(() => {
        ...
        if (appState.activeReadingMode === "voice") {
          const progress = moduleRegistry.voiceRead?.getReadingProgress?.();
          if (progress && Number.isFinite(progress.fraction)) {
            const targetTop = moduleRegistry.voiceRead?.mapProgressToScroll?.(progress.fraction, distance, currentScrollTop);
            if (Number.isFinite(targetTop)) {
              const delta = targetTop - currentScrollTop;
              const step = Math.max(-VOICE_SCROLL_STEP_CAP, Math.min(VOICE_SCROLL_STEP_CAP, delta));
              if (Math.abs(step) >= 0.5) {
                window.scrollBy(0, step);
              }
            }
          } else {
            window.scrollBy(0, this.getVoiceScrollStep(scrollHeight, clientHeight));
          }
          lastScrollTop = currentScrollTop;
          appState.lastScrollPosition = currentScrollTop;
          return;
        }
        const scrollStep = pace.getScrollStepFromDuration(appState.readingDuration, scrollHeight, clientHeight);
        ...
        <div class="control-section">
          ...
          <div class="reading-panel" id="voiceReadPanel" style="display:none;">
            <div class="control-item">
              ...
              <div class="slider-box">
                <input type="range" class="control-slider speed-slider" id="ttsRateSlider" min="0.5" max="2" step="0.1" value="${pace.clampRate(appState.currentScrollSpeed)}">
              </div>
  ...
  function capturePreRenderDom(root) {
    ...
    for (const el of nodes) {
      ...
      if (chunker.isPlausibleText(text)) {
        ...
        extractorState.cachedPreRenderNode = el;
  ...
  function findPhraseInDom(root, normPhrase, fromNormOffset) {
    ...
    while (node = walker.nextNode()) {
      ...
      if (!norm) continue;
      segments.push({ node, raw, norm, normStartInAcc: acc.length });
      acc += norm;
    }
    const matchIndex = acc.indexOf(normPhrase, Math.max(0, fromNormOffset || 0));
  ...
  function findNormOffsetInDom(root, normOffset) {
    ...
    while (node = walker.nextNode()) {
      ...
      const norm = chunker.normalizeText(raw);
      if (!norm) continue;
      if (acc + norm.length > normOffset) {
        return { node, rawOffset: mapNormToRaw(raw, normOffset - acc), normOffset };
      }
      acc += norm.length;
    }
    return null;
  ...
  async function ensurePreRenderDomNode() {
    if (extractorState.cachedPreRenderNode && extractorState.cachedPreRenderNode.isConnected) {
  ...
  const extractor = {
    ...
    async locateTextOffset(normOffset, phrase, fromNormOffset, options = {}) {
      ...
      if (options.probe === false) {
        root = extractorState.cachedPreRenderNode && extractorState.cachedPreRenderNode.isConnected ? extractorState.cachedPreRenderNode : queryPreRenderNodes()[0] || null;
      } else {
        root = await ensurePreRenderDomNode();
      }
      if (!root) return null;
      ...
      if (!found) {
        found = findNormOffsetInDom(root, safeOffset);
      }
  ...
  const speechEngine = {
    ...
    speakChunk(index) {
      ...
      utterance.onend = () => {
        ...
        const durationMs = Math.max(1, Date.now() - this.chunkStartTime - this.pausedTotalMs);
        const chunkText = this.chunks[index] || "";
  ...
  const voiceState = {
    ...
    textLength: 0,
    /** 范围定位：起止文字在整章归一化文本中的偏移与总长（供滚动映射与定位） */
    rangeStartIndex: 0,
    rangeEndIndex: 0,
    rangeTotalLength: 0,
  ...
  const voiceRead = {
    ...
    init() {
      ...
      speechEngine.setHandlers({
        ...
        // chunk 边界是校准点：用预渲染 DOM 实测当前位置，修正线性映射误差
        onChunkStart: (index) => this.scheduleAnchorMeasurement(index),
    ...
    async loadAndSpeak(options = {}) {
      ...
      try {
        ...
        voiceState.rangeStartIndex = rangeResult.startIndex || 0;
        voiceState.rangeEndIndex = rangeResult.endIndex || 0;
        voiceState.rangeTotalLength = rangeResult.totalLength || 0;
        voiceState.rangeStartY = null;
        voiceState.rangeEndY = null;
        voiceState.locatePending = voiceState.rangeStartIndex > 0;
        voiceState.scrollAnchors = [];
        ...
        this.showQuickBar();
        this.syncAllUI();
        this.locateRangeBounds(rangeResult, voiceState.sessionId);
    ...
    /**
     * 当前朗读进度（0~1）。
     * 已完整读过的 chunk 按实际字数累计；当前 chunk 按「已播时长 × 语速 × 每秒字数」估算，
     * 并在每个 chunk 边界（onend 推进 index）自动校准，误差被限制在单个 chunk 内。
     * 供 autoRead 逐帧对齐页面滚动位置，实现“语音读多快、页面滚多快”。
     */
    getReadingProgress() {
      const chunks = speechEngine.chunks || [];
      const total = voiceState.textLength || chunks.reduce((sum, c) => sum + c.length, 0);
      if (!total || !chunks.length) {
        return { fraction: 0, charsRead: 0, totalChars: total || 0 };
      }
      this.collectCpsSamples();
      let charsRead = 0;
      const currentIndex = Math.min(speechEngine.index, chunks.length);
      for (let i = 0; i < currentIndex; i += 1) {
        charsRead += chunks[i].length;
      }
      if (speechEngine.playing && !speechEngine.paused && !speechEngine.stopped && currentIndex < chunks.length) {
        const current = chunks[currentIndex];
        if (this.isBoundaryActive() && speechEngine.boundaryCharIndex >= 0) {
          charsRead += Math.min(current.length * 0.9, speechEngine.boundaryCharIndex);
        } else {
          const cps = this.getEffectiveCps();
          const elapsedSec = (Date.now() - speechEngine.chunkStartTime) / 1e3;
          const estimated = Math.max(0, elapsedSec * cps * ttsSettings.rate);
          charsRead += Math.min(current.length * 0.85, estimated);
        }
      }
      return {
        fraction: Math.min(1, charsRead / total),
        charsRead,
        totalChars: total
      };
    },
    ...
    isBoundaryActive() {
      ...
      if (speechEngine.playing && !speechEngine.paused && speechEngine.chunkStartTime > 0) {
        const elapsed = (Date.now() - speechEngine.chunkStartTime) / 1e3;
        if (elapsed > 2.5) {
    ...
    /** 消费已完成 chunk 的真实耗时，反推该音色的真实 1x 字/秒（滚动平均，安全系数 0.95） */
    collectCpsSamples() {
      ...
      if (voiceState.cpsSamples.length) {
        const avg = voiceState.cpsSamples.reduce((sum, cps) => sum + cps, 0) / voiceState.cpsSamples.length;
        voiceState.calibratedCps = avg * 0.95;
    ...
    scheduleAnchorMeasurement(index) {
      ...
      const last = voiceState.scrollAnchors[voiceState.scrollAnchors.length - 1];
      if (last && charOffset - last.charOffset < 100) return;
      ...
      const total = voiceState.textLength || 0;
    ...
    /** 目标字符偏移：范围区间 [startOffset, endOffset] 内按进度比例定位 */
    getTargetCharOffset(clamped) {
      const startOffset = voiceState.rangeStartIndex || 0;
      const endOffset = voiceState.rangeEndIndex > 0 ? voiceState.rangeEndIndex : Math.max(voiceState.textLength || 0, 1);
      return startOffset + clamped * Math.max(0, endOffset - startOffset);
    },
    /** 线性基线映射：字符偏移 → 滚动像素（按范围起止在整章中的占比） */
    linearMapY(offset, distance) {
      const startOffset = voiceState.rangeStartIndex || 0;
      const endOffset = voiceState.rangeEndIndex > 0 ? voiceState.rangeEndIndex : Math.max(voiceState.textLength || 0, 1);
      const span = Math.max(1, endOffset - startOffset);
      const t = Math.min(1, Math.max(0, (offset - startOffset) / span));
      const startF = this.getRangeStartFraction();
      const endF = this.getRangeEndFraction();
      return distance * (startF + t * Math.max(0, endF - startF));
    },
    ...
    /**
     * 异步定位“从文字/到文字”在正文中的实际位置（预渲染 DOM 实测），
     * 成功后写入 voiceState 并把页面滚动到起点。
     * 定位失败或超时静默降级：不阻塞朗读启动，滚动由比例估算接管。
     */
    async locateRangeBounds(rangeResult, sessionId) {
      const totalLength = rangeResult && rangeResult.totalLength || 0;
      const startIndex = rangeResult && rangeResult.startIndex || 0;
      const endIndex = rangeResult && rangeResult.endIndex || 0;
      if (!totalLength || startIndex <= 0) {
        voiceState.locatePending = false;
        return;
      }
      const startPhrase = chunker.normalizeText(ttsSettings.rangeStart);
      const endPhrase = chunker.normalizeText(ttsSettings.rangeEnd);
      const locate = async (offset, phrase, fromOffset) => {
        if (!offset || offset <= 0 || offset >= totalLength) return null;
        return Promise.race([
          extractor.locateTextOffset(offset, phrase, fromOffset),
          new Promise((resolve) => window.setTimeout(() => resolve(null), 600))
        ]);
      };
      const [startLoc, endLoc] = await Promise.all([
        locate(startIndex, startPhrase, 0),
        locate(endIndex, endPhrase, startIndex + startPhrase.length)
      ]);
      if (sessionId !== voiceState.sessionId) return;
      if (startLoc && Number.isFinite(startLoc.y)) {
        voiceState.rangeStartY = startLoc.y;
        this.addScrollAnchor(startIndex, startLoc.y);
      }
      if (endLoc && Number.isFinite(endLoc.y)) {
        voiceState.rangeEndY = endLoc.y;
        this.addScrollAnchor(endIndex, endLoc.y);
      }
      voiceState.locatePending = false;
      this.scrollToRangeStart();
    },
    /** 定位成功后把页面滚动到“从文字”起点（顶部预留固定栏高度） */
    scrollToRangeStart() {
      const y = voiceState.rangeStartY;
      if (!Number.isFinite(y) || voiceState.locatePending) return;
      window.scrollTo(0, Math.max(0, y - RANGE_SCROLL_TOP_PADDING));
    },
    /**
     * 把朗读进度（相对范围文本 0~1）映射为页面滚动目标（px），供 autoRead 逐帧对齐。
     * - 已定位“从文字”起点：以实测起点 y 为锚，线性滚动到终点 y（未指定结束则到页底）；
     * - 定位失败：按字符占比映射到 [startFraction, endFraction] 区间；
     * - 定位进行中：保持当前位置，避免“先跳比例位置、再跳精确位置”的两次跳变。
     */
    mapProgressToScroll(fraction, distance, currentTop) {
      if (voiceState.locatePending) return currentTop;
      const clamped = Math.min(1, Math.max(0, Number(fraction) || 0));
      const safeDistance = Math.max(0, distance);
      const targetOffset = this.getTargetCharOffset(clamped);
      const linearY = this.linearMapY(targetOffset, safeDistance);
      const err = this.interpolateAnchorErr(targetOffset);
      return Math.min(safeDistance, Math.max(0, linearY + err));
    },
    /** “从文字”在整章中的占比（比例估算用） */
     ...
     * 优先级：预渲染 DOM 实测定位 → 按字符占比的比例估算；找不到则仅提示不滚动。
    ...
    async jumpToRangeStart() {
      ...
      const located = await Promise.race([
        extractor.locateTextOffset(startIndex, chunker.normalizeText(startText), 0),
        new Promise((resolve) => window.setTimeout(() => resolve(null), 600))
      ]);
      if (token !== voiceState.jumpToken) return;
      if (located && Number.isFinite(located.y)) {
        window.scrollTo(0, Math.max(0, located.y - RANGE_SCROLL_TOP_PADDING));
        utils.notificationManager.show("已定位到开始文字");
        return;
      }
      if (totalLength > 0) {
        ...
        const fraction = Math.min(1, Math.max(0, startIndex / totalLength));
        window.scrollTo(0, Math.max(0, distance * fraction));
