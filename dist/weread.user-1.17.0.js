  ...
  function findNormOffsetInDom(root, normOffset) {
    ...
    while (node = walker.nextNode()) {
      ...
      if (acc + norm.length > normOffset) {
        return { node, rawOffset: mapNormToRaw$1(raw, normOffset - acc), normOffset };
      }
      acc += norm.length;
    }
    return null;
  ...
  const voiceRead = {
    ...
    async prepareLayoutMap(sessionId, signal) {
      ...
      const [startLoc, endLoc] = await Promise.all([
        locate(voiceState.rangeStartIndex),
        locate(Math.max(voiceState.rangeStartIndex, voiceState.rangeEndIndex - 1))
      ]);
      if (signal?.aborted || sessionId !== voiceState.sessionId) return false;
