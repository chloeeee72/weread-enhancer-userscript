  ...
  const autoRead = {
    ...
    calculateWaitTime() {
      if (appState.activeReadingMode === "auto") {
        ...
        return pace.getPageTurnWaitFromDuration(PAGE_DURATION_AT_1X / speed * 10);
      }
    ...
    beginScroll() {
      ...
      appState.scrollInterval = setInterval(() => {
        ...
        const scrollStep = this.isDoubleColumnReading() ? pace.getScrollStepFromDuration(appState.readingDuration, scrollHeight, clientHeight) : pace.getScrollStepFromSeconds(PAGE_DURATION_AT_1X / Math.min(2, Math.max(0.1, appState.autoScrollSpeed || 1)) * 10, scrollHeight, clientHeight);
